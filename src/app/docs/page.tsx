        ...
        {MANUAL_SLUGS.map((slug) => {
          ...
          if (!enabled)
            return (
              <div
                ...
                aria-disabled="true"
                title="권한이 없습니다"
                className="flex cursor-not-allowed items-center justify-center gap-1.5 rounded-lg border border-line bg-paper-2 p-4 opacity-50"
              >
          ...
          return (
            <Link
              key={slug}
              href={`/docs/${slug}`}
              className="flex items-center justify-center rounded-lg border border-line p-4 transition-colors hover:border-clay hover:bg-paper-2"
            >
