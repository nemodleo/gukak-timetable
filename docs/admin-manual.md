...
- **승인 지정** *(관리자 전용)* — 눌러서 켠 뒤 날짜 칸 위를 드래그하면, 지나간 날짜들을 한 번에
  승인(오픈)하거나 다시 잠급니다. 새로 생기는 날짜는 기본이 **잠금**이라 강사가 편집할 수 없고
  자물쇠 아이콘이 뜹니다 — 이 기능이 생기기 전부터 이미 데이터가 있던 날짜는 처음부터 승인된 상태입니다.
- **이번 달 전체 승인 / 이번 달 전체 잠금** *(관리자 전용)* — 그 달에 보이는 모든 날짜(스필오버로
  보이는 이웃 달 날짜는 제외)를 한 번에 승인하거나 다시 잠급니다. 달이 바뀔 때 한 번에 열어두기
  편합니다.
...
- 로그인 상태(강사·관리자)에서는 시간표 옆(화면이 좁으면 아래)에 그 달 **목표 차이** 요약이 함께
  보입니다 — 통계 탭까지 가지 않아도 바로 확인할 수 있습니다.
...
> **드래그로 칸 옮기기** — 이미 배정된 칸을 마우스로 눌러 다른 칸(같은 날짜 안 다른 시간·강의실)으로
> 끌어 놓으면 배정이 그대로 옮겨갑니다(강사도 가능). 대상 칸에 이미 배정이 있으면 덮어쓸지 확인합니다.
...
아침·저녁처럼 넓은 구간을 막을 때 편합니다. **시간(행) 라벨**이나 **강의실(열) 이름**을 클릭하면 그
시간대 전체(모든 방) 또는 그 강의실 전체(하루 전체)를 한 번에 토글할 수도 있습니다.

이미 학생이 배정된 칸을 비활성으로 지정해도 **배정 내용은 지워지지 않습니다** — 회색 빗금으로
표시되고 강사는 더 이상 그 칸을 수정할 수 없게 되지만(관리자는 계속 수정 가능), 학생·시수 통계에는
비활성 여부와 무관하게 그대로 집계됩니다. 다시 활성화하면 원래 색과 입력 권한만 돌아옵니다.

> **자동 저장** — 칸 편집에는 따로 "전체 저장" 버튼이 없습니다. 저장을 누르거나 드래그를 끝내는 순간
> 바로 반영됩니다.

### 5.6 날짜 승인(강사 입력 잠금 해제) · *관리자 전용*

날짜 제목 옆에 **자물쇠 아이콘 + 승인 대기** 배지가 보이면 그 날은 아직 관리자가 승인하지 않아 강사가 배정·메모를
편집할 수 없습니다(보기는 가능). 관리자에게는 이 배지가 **버튼**으로 보이며, 클릭 한 번으로 그 날
하나만 바로 승인(승인됨)하거나 다시 잠글 수 있습니다. 여러 날짜를 한 번에 다루려면 월간 달력의
**승인 지정** 드래그를 쓰세요(3장 참고). 관리자 본인은 잠금과 무관하게 항상 편집할 수 있습니다.
...
- **강의실** — 이름을 클릭해 바꾸고, 칸에 마우스를 올리면 나타나는 **✕** 로 삭제, 맨 끝 **+** 로 추가.
  **이름을 드래그하면 강의실 순서를 바꿀 수 있습니다**(칸 배정은 이름 기준이라 그대로 유지됩니다).
- **시간블록** — 시간 라벨을 클릭해 시작·끝을 수정, **✕** 로 삭제, **+** 로 추가. 새 블록은 *직전 블록
  끝 + 2시간*으로 생기고, 다른 블록과 **겹치지 않도록** 자동 조정됩니다.
  **시간 라벨을 드래그하면** 그 블록을 다른(빈) 시간으로 통째로 옮기고(길이는 유지), **다른 블록 위에
  놓으면** 두 블록의 시간을 서로 맞바꿉니다 — 배정된 칸도 함께 이동합니다.
...
- *+ 행 추가* / 각 행 오른쪽 *삭제*. 행 왼쪽의 **점 6개 손잡이를 드래그**하면 행 순서를 자유롭게
  바꿀 수 있습니다.
...
- **강의실 목록**, **평일·주말 기본 시간블록** — 날짜별로 따로 바꾸지 않은 날은 이 값을 씁니다
  (엑셀 "SET UP" 시트에 해당).
- **색 팔레트** — 1학년·2학년·3학년·보강·합반 4가지 색을 직접 고를 수 있습니다. 색상 피커 또는
  `#rrggbb` 텍스트로 입력하면 학년 배지·배정 칸 색이 전부 그 색을 기준으로 바뀝니다(칸 배경은 고른
  색보다 옅게 자동 계산). *기본값으로*로 원래 색으로 되돌릴 수 있습니다.
- **비활성 칸 색 · 무늬** — 배정 없는 칸/비활성 지정된 칸의 **배경색**(무늬 사이 여백, *단색*
  무늬일 땐 칸 전체를 채우는 색)과 **패턴색**(선·점 색)을 따로 고르고, 무늬(사선·격자·점·단색)도
  선택합니다. 무늬 4개는 미리보기가 그대로 실제 칸에 적용됩니다.
...
### 수업 시간 옮기기
- 가장 빠른 방법: 배정된 칸을 **마우스로 눌러 원하는 칸으로 드래그**.
- 또는: 1) 새 칸을 클릭해 그 학생을 배정하고 *저장*. 2) 원래 칸을 클릭 → *칸 비우기*.

### 비활성(수업 없는) 시간 지정
- 넓게: *편집* → 툴바 *비활성 지정* 켜고 드래그, 또는 시간/강의실 이름 클릭.
- 한 칸: 칸 클릭 → *비활성* 탭 → (필요하면 메모·색) → *저장*.
- 이미 배정된 칸을 비활성 지정해도 배정은 지워지지 않는다 — 회색으로 바뀌고 강사 입력만
  막히며, 통계엔 계속 집계된다.
...
| 날짜 승인 | 관리자가 그 날짜를 "열어" 강사가 편집할 수 있게 하는 것. 기본은 잠금. |
